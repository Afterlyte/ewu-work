...
public class Graph {
	...
	public void printPath(int v) {
		...
		for(LinkedList l : paths) {
			if(l != null) {
				res += "[" + l.getLast()[0] + "] ";
				if(!l.contains(v)) res += "unreachable\n";
				else {
					res += "shortest path:(" + l.getFirst()[0];
					for(int i = 1; i < l.getSize(); i++) {
						res += "," + l.getAtIndex(i)[0];
					}
					int vertex = l.getLast()[0];
					res += ") shortest distance:" + this.shortest.getWeight(vertex) + "\n";
				}
			}
		}
