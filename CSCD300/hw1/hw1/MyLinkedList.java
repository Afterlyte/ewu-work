...
public class MyLinkedList <E extends Comparable<? super E>>{
	...
	public boolean removeAllCopies( E o ) { //passed test
		ListNode cur = this.head.next, prev = this.head;
		...
		return success; //change this as you need.
	}
	...
	public MyLinkedList<E> interleave(MyLinkedList<E> A) {
		if(A.isEmpty()) {
			return this;
		}
		MyLinkedList<E> result = new MyLinkedList<E>();
		ListNode cur1 = this.head.next, cur2 = A.head.next;
		while(cur1 != null) {
			result.add(cur1.data);
			result.size++;
			if(cur2 != null) {
				result.add(cur2.data);
				result.size++;
			}
		...
		return result; //change this as you need.
	}
