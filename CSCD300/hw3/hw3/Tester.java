...
public class Tester {
	...
	public int countSpace(String str) {
		...
		if(current == ' ') {
			...
			return 0 + countSpace(str.substring(1));
		}
	}
	...
	private boolean isSum24(int arr[], int targetSum) {
		...
		else if(arr.length == 1 && arr[0] != targetSum){
			return false;
		}
		...
		return isSum24(arrSmall, targetSum - arr[arr.length - 1]); //change this line of code as needed.
	}
	...
	{
		int min = low;
		for(int search = low + 1; search <= high; search++) {
			if(a[search] < a[min]) {
				min = search;
			}
		}
